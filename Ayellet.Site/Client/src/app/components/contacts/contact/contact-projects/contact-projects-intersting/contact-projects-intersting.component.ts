...
import { ContactProjectsVolunteerComponent } from '../contact-projects-volunteer/contact-projects-volunteer.component';
...
@Component({
    selector: 'contact-projects-intersting',
    templateUrl: './contact-projects-intersting.component.html',
    //styleUrls: ['contact-projects-intersted.component.css']
})

export class ContactProjectsInterstingComponent implements OnInit {
    private currentcontact: any;
    private contactId: number;
    private projects: ProjectInterested[] = [];
    private currentContact: Contact;
    private currentIsActive: boolean;
    private selectedRow: Number = -1;
    private newProject: ProjectInterested;
    private flagAddition: boolean;
    private lengthArrry: number;
    private listArea: Area[] = [];
    private selectedArea: Area;
    private selectedProject: ProjectInterested;
    private flag: string;
    ...
    GetProjectsById(contactId: number) {
        this.contactsService.getProjectsInterestedByIdContact(contactId).then(project => {
            console.log('project');
    ...
    showAddDialog() {
        ...
        let disposable = this.dialogService.addDialog(AddContactDialogComponent, {
            selectedProjectI: this.selectedProject,
            active: this.status,
            ...
            .subscribe((result) => {
                ...
                if (!result) {
                    ...
                    /*if (this.validateRow()) {
                        if (this.status == 2) {
                            this.newProject.idProbectIntersted = -1;
                    ...
                    if (this.status == 1) {
                        this.contactsService.updateProjectDetiales(this.selectedProject).then(answer => {
                            this.editAnswer = answer;
                            if (this.editAnswer) {
                                alert("עודכן בהצלחה");
                                window.location.reload();
    ...
    validateRow(): boolean {
        if (this.status == 2) {
            ...
            if (this.selectedProject.date != null && this.selectedProject.details != '' && this.selectedProject.roleInterested != '') {
                return true;
            }
        }
        alert('כישלון עידכון')
    ...
    validateNewRow(): boolean {
        if (this.newProject.date == null || this.newProject.details == '' || this.newProject.roleInterested == '') {
            alert('חדש עידכון')
            return false;
        }
        return true;
